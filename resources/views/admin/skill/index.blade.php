...
@section('konten')
	<div class="container-fluid">
		<div class="row">
			<div class="col-md-12">
				<h1 class="page-header">Data <label class="label label-danger">skill</label></h1>
			</div>
		<!--  -->
			<div class="col-md-12">
				<a href="{{ url('admin') }}" class="btn btn-danger">
					<i class="fa fa-arrow-left fa-lg"></i>  Kembali</a>
				<a href="{{ action('skillkategoricontrol@index') }}" class="btn btn-info pull-right">
					<i class="fa fa-server fa-lg"></i>  Kategori</a>
				<a href="{{ action('skillcontrol@create') }}" class="btn btn-success pull-right">
					<i class="fa fa-plus-circle fa-lg"></i>  New skill</a>
			</div>
		<!--  -->
			<div class="col-md-12">
				<hr>
				<div class="table-responsive">
					...
					@foreach($data as $data)
							<tr>
								...
								<td>{{ $data->nama }}</td>
								<td>
								<?php $t = strtotime($data->created_at) ?>Tambah, {{ date('d M y. h:i A',$t) }}<br>
								<?php $t = strtotime($data->updated_at) ?>Update, {{ date('d M y. h:i A',$t) }}
								</td>
								<td>
								<ul class="nolis">
									<li><a href="{{ url('admin/skill/'. $data->id .'/edit') }}" class="btn label label-info lbl" title="Sunting">
										<i class="fa fa-edit fa-lg"></i></a></li>
									<li>|</li>

									<li><a href="{{ action('skillcontrol@show', $data->id) }}" class="btn label label-success lbl" title="Lihat" target="blank"><i class="fa fa-eye fa-lg"></i></a></li>
									<li>|</li>

									<form class="form-upt" method="post"
										action="{{ action('skillcontrol@update',$data->id) }}">
										<li>
										{{ csrf_field() }}
			        					{{ method_field('PUT') }}
								...
								<td>
									<form class="form-del" method="post" 
										action="{{ action('skillcontrol@destroy',$data->id) }}">
			        					{{ csrf_field() }}
			        					{{ method_field('DELETE') }}
