...
@section('konten')
	<div class="container-fluid">
		<div class="row">
			<div class="col-md-12">
				<h1 class="page-header">Data <label class="label label-info">Artikel</label></h1>
			</div>
		<!--  -->
			<div class="col-md-12">
				<div class="row">
				...
				<!--  -->
					<div class="col-sm-8">
						<a href="{{ action('blogkategoricontrol@index') }}" class="btn btn-info pull-right">
							<i class="fa fa-server fa-lg"></i>  Kategori</a>

						<a href="{{ action('blogcontrol@create') }}" class="btn btn-success pull-right">
							<i class="fa fa-plus-circle fa-lg"></i>  New Artikel</a>	
					</div>
				</div>
			</div>
		<!--  -->
				...
				<div class="table-responsive">
					...
					@foreach($data as $data)
							<tr>
								...
								<td>{{ $data->nama }}</td>
								<td>
								<?php $t = strtotime($data->created_at) ?>Create, {{ date('d M y. h:i A',$t) }}<br>
								<?php $t = strtotime($data->updated_at) ?>Update, {{ date('d M y. h:i A',$t) }}
								</td>
								<td>
								<ul class="nolis">
									<li><a href="{{ url('admin/blog/'. $data->id .'/edit') }}" class="btn label label-info lbl" title="Sunting">
										<i class="fa fa-edit fa-lg"></i></a></li>
									<li>|</li>

									<li><a href="{{ action('blogcontrol@show', $data->id) }}" class="btn label label-success lbl" title="Lihat" target="blank"><i class="fa fa-eye fa-lg"></i></a></li>
									<li>|</li>

									<form class="form-upt" method="post"
										action="{{ action('blogcontrol@update',$data->id) }}">
										<li>
										{{ csrf_field() }}
			        					{{ method_field('PUT') }}
								...
								<td>
									<form class="form-del" method="post" 
										action="{{ action('blogcontrol@destroy',$data->id) }}">
			        					{{ csrf_field() }}
			        					{{ method_field('DELETE') }}
