...
@section('nav')
	<ul class="pager">
		<li class="previous">
			@if($net == null)
			<a href="{{url('framework')}}" 
				data-toggle="tooltip" data-placement="right" title="Framework">
					<i class="fa fa-arrow-circle-o-left fa-lg"></i>  Prev</a>
			@elseif($net->slugside == 'index')
			<a href="{{url('framework',$nama)}}" 
				data-toggle="tooltip" data-placement="right" title="{{$nama}}">
					<i class="fa fa-arrow-circle-o-left fa-lg"></i>  Prev</a>
			@else
			<a href="{{url('framework/'.$nama.'/'.$net->slugside)}}" 
...
@section('konten') @include('layouts.iklan')
	<div class="container-fluid">
		...
		<!-- Side Bar -->
			<div class="col-md-3 col-md-pull-6 col-sm-6 pd-30">
				<div class="form-group">
					<a href="{{url('framework')}}" class="btn btn-primary btn-block btn-border btn-lg" title="Framework">Framework</a>
				</div>
					...
					<div class="panel panel-primary app-panel">
						...
						@foreach($subkat as $subkat)
							...
							@elseif($subkat->slugside == $slug)
								<a href="{{url('framework/'.$nama.'/'.$subkat->slugside)}}"
									data-toggle="tooltip" data-placement="top" title="{{$subkat->sidejudul}}"
										class="list-group-item active">{{$subkat->sidejudul}}</a>
							@else
								<a href="{{url('framework/'.$nama.'/'.$subkat->slugside)}}"
