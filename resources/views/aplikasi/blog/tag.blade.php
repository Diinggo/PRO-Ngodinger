...
@section('judul')#{{ $slug }} - Tags Artikel | Ngodinger @endsection
		...
		<!--  -->
			...
			<div class="panel panel-primary app-panel">
				<div class="panel-body">
					<div class="row">
						<div class="col-sm-4">
							<a href="{{url('blog',$data->slug)}}" data-toggle="tooltip" data-placement="bottom" title="{{$data->judul}}" class="block">
								<img src="{{$data->foto}}" alt="{{$data->judul}}" class="img-tag">
							</a>
						</div>
						<div class="col-sm-8">
							<h4 class="nover-tam judul-tag"><a href="{{url('blog',$data->slug)}}" data-toggle="tooltip" data-placement="bottom" 
								title="{{$data->deskripsi}}">{{$data->judul}}</a></h4>
							<h5 class="tag-rep">
							<?php $tags = explode(',', $data->keyword); $sluh = str_replace('-',' ',$slug); ?>
							@foreach($tags as $tag)
							@if($tag == $sluh)
								<a href="{{ url('blog/tag/'.str_slug($tag)) }}" data-toggle="tooltip" data-placement="top" title="{{$tag}}" 
